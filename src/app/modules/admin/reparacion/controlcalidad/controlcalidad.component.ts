...
export class ControlcalidadComponent implements OnInit {
    ...
    // Combina todas las preguntas en un solo arreglo (cómodo)
    private get todasLasPreguntas() {
        return [...this.preguntasIzquierda, ...this.preguntasDerecha];
    }
    ...
    iniciar(rowData: any) {
        ...
        this.respuestas = {};
        for (const p of [
            ...this.preguntasIzquierda,
            ...this.preguntasDerecha,
        ]) {
            this.respuestas[p.campo] = true; // ✅ siempre "Sí"
        }
    ...
    guardarQC() {
        // 1) Validar que respondieron todo
        if (!this.todasRespondidas()) {
            this.messageService.add({
                severity: 'warn',
                summary: 'Faltan respuestas',
                detail: 'Responda todas las preguntas antes de guardar.',
                life: 3000,
            });
            return;
        }

        // 2) Si todas son "Sí" -> aprobar, si alguna es "No" -> desprobar
        const aprobado = this.todasAprobadas();
        ...
        this.reparacionService.aprobarQC(dto).subscribe({
            next: (res) => {
                if (res.res) {
                    this.messageService.add({
                        severity: aprobado ? 'success' : 'info',
                        summary: aprobado ? 'QC Aprobado' : 'QC Desaprobado',
                        detail: res.mensaje,
                        life: 3000,
                    });
                    this.visible = false;
                    this.buscar(); // refresca lista
                } else {
                    this.messageService.add({
                        severity: 'warn',
                        summary: 'Advertencia',
    ...
    todasRespondidas(): boolean {
        return this.todasLasPreguntas.every(
            (p) => this.respuestas[p.campo] !== null
        );
    }

    todasAprobadas(): boolean {
        return this.todasLasPreguntas.every(
            (p) => this.respuestas[p.campo] === true
        );
    }
}
